...
public class MainMessage_4_Info_1 extends BaseActivity implements AbsListView.OnScrollListener {
    ...
    void onDataHttpAdvicesRELists(String response, String type) {
        if (type.equals("0")) {
            ...
            try {
                ...
                String jsonArray_str = jsonObject_response.getString("data");
                if (jsonArray_str.equals("[]") || jsonArray_str.equals("null")) {
                    MyToast.showTheToast(activity, "我是有底线的～");
                } else {
                    JSONArray jsonArray = jsonObject_response.getJSONArray("data");
                    for (int i = 0; i < jsonArray.length(); i++) {
                        JSONObject jsonObject = jsonArray.getJSONObject(i);
