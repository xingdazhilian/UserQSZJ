...
public class MainMessage_4_Info_2 extends BaseActivity implements AbsListView.OnScrollListener {
            ...
            try {
                ...
                if (status.equals("1")) {
                    ...
                    if (data_str.equals("[]")) {
                        ...
                        myBaseAdapter = new MyBaseAdapter(activity, dataList, R.layout.main_message_4_info_item_2);
                        listview.setAdapter(myBaseAdapter);
                        listview.setDividerHeight(0);

                        listview.setOnItemClickListener(new AdapterView.OnItemClickListener() {
                            @Override
                            public void onItemClick(AdapterView<?> parent, View view_1, int position, long id) {

                                HttpIsRead(dataList.get(position).get("id").toString());

                                String jingweidu = dataList.get(position).get("jwd");
                                ...
                                if (!jingweidu.equals("")) {
                                    ...
                                    View view = inflater.inflate(R.layout.alertdialog_message_map, null);

                                    String[] all = jingweidu.split(",");

                                    double Dwei = Double.parseDouble(all[1]);
                                    double Djing = Double.parseDouble(all[0]);
                                    LatLng marker = new LatLng(Dwei, Djing);
                                    MarkerOptions markerOptions = new MarkerOptions().position(marker).draggable(true);
                                    ...
                                    aMap.animateCamera(CameraUpdateFactory.newCameraPosition(new CameraPosition(marker, 13, 0, 0)), 1000, null);
